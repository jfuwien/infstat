...
\tsbook{Studierendenmitschrift zur Vorlesung: Einführung in die Inferenzstatistik (SoSe 2021)}
       {Jim Feller}
       {Cover Designer}
       {2017}
...
\begin{example}[Ein Modell für den radioaktiven Zerfall]
	...
	\begin{tikzpicture}[overlay, remember picture,shorten <=1mm,
		nodes={inner sep=1pt, align=center, font=\footnotesize}]
		\draw[<-] (F.south) -- ++ (0,-0.7) node[below] {$ n \to \infty $ \\ $ p \to 0 $, \\ sodass $ np \to \lambda $ }; 
	\end{tikzpicture}
		\vspace{8ex}
